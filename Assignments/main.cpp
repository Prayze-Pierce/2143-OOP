...
int main() {
    ...
    if (!file.is_open()) { std::cerr << "Error: Could not open shapes.json\n"; return 1; }

    json data;
    try { file >> data; }
    catch (const std::exception& e) { std::cerr << "JSON parse error: " << e.what() << "\n"; return 1; }

    if (!data.contains("shapes")) { std::cerr << "Error: JSON missing 'shapes' key\n"; return 1; }
    ...
    if (!shapes_data.contains(choice)) { std::cerr << "Shape not found.\n"; return 1; }
